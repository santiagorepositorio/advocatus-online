    ...
    <div class="grid grid-cols-3 divide-x divide-gray-200">
        ...
        <div class="col-span-2">
            ...
            <div class="bg-gray-100 h-16 flex items-center px-4">
                ...
                <h2 class="text-teal-600 text-lg ml-4">{{ auth()->user()->name }}</h2>
            </div>
            ...
            <div class="h-[calc(100vh-10.5rem)] overflow-auto border-t border-gray-200">
                ...
                <div class="px-4 py-3">
                    <h2 class="text-teal-600 text-lg mb-4">REMITENTE(S)</h2>
                    <div x-data="{ tab: @entangle('tab') }">
                        {{-- Pestañas --}}
                        <div class="mb-4">
                            <button @click="tab = 1" :class="{ 'bg-blue-500 text-white': tab === 1 }"
                                wire:click="changeTab(1)" class="py-2 px-4 text-black">Escribir Números</button>
                            <button @click="tab = 2" :class="{ 'bg-blue-500 text-white': tab === 2 }"
                                wire:click="changeTab(2)" class="py-2 px-4 text-black">Tabla de Usuarios</button>
                        </div>

                        {{-- Contenido de las pestañas --}}
                        <div x-show="tab === 1">
                            {{-- Contenedor para escribir números de celular --}}
                            <label for="phoneInput" class="block mb-2">Escribe números de celular:</label>
                            <input type="text" id="phoneInput" wire:model="phoneNumbers"
                                class="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-blue-300">
                            <button wire:click="addPhoneNumbers">Agregar Números</button>
                        </div>
                        ...
                        <div x-show="tab === 2">
                            ...
                            @if (count($users))
                                <table class="w-full border border-gray-300">
                                    ...
                                    <tbody>
                                        {{-- Itera sobre tus usuarios y muestra sus datos --}}
        ...
        <div class="col-span-1">
            <div class="bg-gray-100 h-16 flex items-center px-4">
                <h2 class="text-teal-600 text-lg mb-4 mr-4">PLANTILLAS</h2>
                <div class="flex-1">
                    <select wire:model="selectedTemplate" id="templateSelect"
                        class="block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:border-blue-300">
            ...
            <div class="h-[calc(100vh-11rem)] px-3 py-2 overflow-auto">
                @if ($selectedTemplate)
                    <div class="mt-4">
                        ...
                        @php
                            $selectedTemplateDetails = collect($templates)->firstWhere('id', $selectedTemplate);
                        @endphp

                        {{-- Mostrar detalles del template seleccionado --}}
                        <h3>Detalles del Template</h3>
                        <p>ID: {{ $selectedTemplateDetails['id'] }}</p>
                        <p>Nombre: {{ $selectedTemplateDetails['name'] }}</p>
                        <p>Categoría: {{ $selectedTemplateDetails['category'] }}</p>
                        <p>Estado: {{ $selectedTemplateDetails['status'] }}</p>
                        <p>Idioma: {{ $selectedTemplateDetails['language'] }}</p>

                        <h4>Componentes</h4>
                        @foreach ($selectedTemplateDetails['components'] as $component)
                            <p class="font-bold">Tipo: {{ $component['type'] }}</p>
                            @if (isset($component['format']))
                                <p>Formato: {{ $component['format'] }}</p>
                            @endif

                            {{-- Verificar si la clave 'text' existe antes de intentar acceder a ella --}}
                            ...
                            @if ($component['type'] == 'BUTTONS' && isset($component['buttons']))
                                <h5 class="font-bold">Botones</h5>
                                @foreach ($component['buttons'] as $button)
                                    <p>Tipo de botón: {{ $button['type'] }}</p>

                                    {{-- Verificar si la clave 'text' existe antes de intentar acceder a ella --}}
                                    @if (array_key_exists('text', $button))
                                        <p>Texto del botón: {{ $button['text'] }}</p>
                                    @endif

                                    @if ($button['type'] == 'URL' && array_key_exists('url', $button))
                                        <p>URL: {{ $button['url'] }}</p>
                                    @endif
            ...
            {{-- <div class="w-full h-full flex justify-center items-center">
                ...
                <div>
                    ...
                    <div class="WM0_u" style="transform: scale(1); opacity: 1;">
                        <span data-testid="intro-md-beta-logo-light" data-icon="intro-md-beta-logo-light"
