            ...
            <div class="card-body bg-gray-100">
                @if ($experience->id == $item->id)
                    <form wire:submit.prevent="update">
                        <input wire:model="experience.title" class="form-input w-full mb-2">
                        @error('experience.title')
                            <span class="text-xs text-red-500">{{ $message }}</span>
                        @enderror
                        <input wire:model="experience.institution" class="form-input w-full mb-2">
                        @error('experience.institution')
                            <span class="text-xs text-red-500">{{ $message }}</span>
                        @enderror
                        <input wire:model="experience.gestion" class="form-input w-full mb-2">
                        @error('experience.gestion')
                            <span class="text-xs text-red-500">{{ $message }}</span>
                        @enderror
